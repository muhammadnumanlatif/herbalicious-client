...
export default function SolutionQuiz() {
    ...
    const handleSelect = (option: any) => {
        ...
        if (currentStepIndex === 0) {
            const nextStep = quizSteps.find(s => (s as any).depends === option.id);
            if (nextStep) {
    ...
    return (
        <section className="py-5 bg-primary-subtle rounded-5 mx-2 mx-md-5 my-5 shadow-sm border border-white position-relative overflow-hidden">
            ...
            <div className="position-absolute top-0 start-0 w-100 h-100" style={{ opacity: 0.05, pointerEvents: 'none' }}>
                <div className="position-absolute" style={{ top: '-50px', right: '-50px', width: '200px', height: '200px', borderRadius: '50%', background: 'var(--bs-primary)' }}></div>
                <div className="position-absolute" style={{ bottom: '-50px', left: '-50px', width: '150px', height: '150px', borderRadius: '50%', background: 'var(--bs-secondary)' }}></div>
            </div>
                                        ...
                                        {currentStep.options.map((option) => (
                                            <Col md={4} key={option.id}>
                                                <MotionDiv
                                                    ...
                                                    className="p-4 bg-white rounded-4 shadow-sm border cursor-pointer h-100 d-flex flex-column align-items-center justify-content-center transition-all hover-prime"
                                                >
                                    ...
                                    <Card className="border-0 shadow-lg rounded-5 overflow-hidden mx-auto mb-4" style={{ maxWidth: '400px' }}>
                                        ...
                                        <Card.Body className="p-4 bg-white">
                                            ...
                                            <div className="d-grid gap-2">
                                                <Link
                                                    href={`/shop?search=${recommendedProduct.id}`}
                                                    className="btn btn-primary rounded-pill py-2 fw-bold"
                                                >
                                                    Inquire via WhatsApp <FaArrowRight className="ms-2" />
                                                </Link>
                                                <Button
                                                    variant="link"
                                                    className="text-muted text-decoration-none shadow-none"
                                                    onClick={resetQuiz}
                                                >
